...
♂ - абит ссылки/начальности (префиксный оператор)
♀ - абит значения/конечности (постфиксный оператор)
→ - связь (∞♀ → ♂∞)
, - несвязь (♂∞ → ∞♀)
...
class MTCAbitNotation:
    ...
    ABITS = {
        'reference_start': '♂',    # абит ссылки/начальности (префикс)
        'value_end': '♀',          # абит значения/конечности (постфикс)
        'connection': '→',         # связь (∞♀ → ♂∞)
