...
`□` означает шаблонный слот: связь-кандидат текущего исполнения шаблона. Внутри одного исполнения все одинаковые вхождения `□` относятся к одной и той же связи-кандидату, поэтому акорень ищет один `X`, для которого:
...
`□` не является пустым множеством, пустым результатом поиска, несвязью или `0`. Фигурные скобки `{}` закреплены за [пучковой нотацией](docs/specs/Пучковая%20нотация%20МТС.md). Квадратные скобки `[` и `]` остаются в слое ачисел и сериализации; они не используются как группировка или экранирование формул.
...
- [Шаблонный поиск МТС](docs/specs/Шаблонный%20поиск%20МТС.md) — правила `□`, `⟦s⟧N` и исполнения шаблона.
- [Пучковая нотация МТС](docs/specs/Пучковая%20нотация%20МТС.md) — фигурные скобки `{}` как пучок связей.
