# Метатеория связей (МТС)

## Что сейчас является МТС

МТС строится вокруг постулата:

```text
всё есть связь
```

Текущая рабочая версия разделяет уровни, которые нельзя смешивать:

```text
связь в натуре
→ смысл связи
→ знак смысла
→ формальная запись смысла
→ шаблон поиска формы
→ возможная сериализация в ачисло
```

Рабочая рамка:

```text
Ачисла описывают структуры связей.
Формальная нотация МТС описывает смыслы и шаблоны смыслов.
Система аксиом МТС описывает виды форм смыслов.
```

Ключевая формула акорня:

```text
∞ : {} = {} ⟼ {}
```

`{}` означает шаблонный поиск без фильтра: связь-кандидат текущего исполнения шаблона. Внутри одного исполнения все одинаковые вхождения `{}` относятся к одной и той же связи-кандидату. Поэтому формула акорня ищет один `X`, для которого:

```text
X = X ⟼ X
```

После исполнения допустима формула:

```text
∞ = ∞ ⟼ ∞
```

`{}` не является пустым множеством, пустым результатом поиска, несвязью или `0`. Квадратные скобки `[` и `]` остаются в слое ачисел и сериализации; они не используются как группировка или экранирование формул.

## Где система аксиом

Актуальное ядро описано в [Системе аксиом МТС](docs/theory/Система%20аксиом%20МТС.md). Каждая аксиома оформляется по [Шаблону аксиом МТС](docs/theory/Шаблон%20аксиом%20МТС.md).

Минимальный рабочий набор:

```text
∞ : {} = {} ⟼ {}
∞ = ∞ ⟼ ∞

♂{} : ♂{} = ♂{} ⟼ {}
♂∞ = ♂∞ ⟼ ∞

{}♀ : {}♀ = {} ⟼ {}♀
∞♀ = ∞ ⟼ ∞♀

{[} : ∞♀
{]} : ♂∞
{⟼} : {[} ⟼ {]}
```

## Где формальная нотация

- [Основания МТС](docs/theory/Основания%20МТС.md) — короткое основание текущей рамки.
- [Метатеория связей](docs/theory/Метатеория%20связей.md) — связное изложение рабочей версии.
- [Формальная нотация МТС](docs/specs/Формальная%20нотация%20МТС.md) — реестр символов и рабочий синтаксис.
- [Шаблонный поиск МТС](docs/specs/Шаблонный%20поиск%20МТС.md) — правила `{}` и исполнения шаблона.
- [Слои нотации МТС](docs/specs/Слои%20нотации%20МТС.md) — разделение смысловой нотации, ачисел и асети.
- [Ачисла и сериализация](docs/specs/Ачисла%20и%20сериализация.md) — статус `[`, `]`, `1`, `0`.
- [Открытые вопросы новой аксиоматики МТС](docs/research/Открытые%20вопросы%20новой%20аксиоматики%20МТС.md) — вопросы, которые не входят в принятое ядро.

## Где тест формул

Канонический файл формул:

```text
tests/mtc_formulas.mtc
```

Он содержит только формулы МТС и комментарии. В нём нет служебных префиксов `AXIOM`, `EXEC`, `RULE`, `NEGATIVE`, `STATUS`.

Регрессионные проверки:

```text
tests/test_issue42_docs.py
```

Они проверяют:

- наличие актуальных документов;
- чистый формат `tests/mtc_formulas.mtc`;
- наличие обязательных формул;
- правило связывания повторных `{}`;
- отделение `{}` от `{s}`;
- отсутствие скобочной записи связи как ядра;
- согласованность ссылок README.

## Как проверить согласованность

```bash
python -m pytest tests -v --tb=short
ruff check parsers/ converters/ core/ tests/ --ignore E501,F401
git diff --check
```

Минимальная проверка конвертеров:

```bash
python -m pytest tests/test_converters.py -v
```

## Инструменты

```text
converters/text_to_anum.py      UTF-8 текст → четверичное ачисло
converters/anum_to_text.py      четверичное ачисло → UTF-8 текст
converters/ascii_unicode.py     ASCII ↔ Unicode нотация
parsers/mtc_formula_prover.py   практический прувер поддерживаемых формул
core/axioms/validate_axioms.py  валидация текущих кодовых инвариантов
```

Кодовые инструменты сохраняют практические проверки сериализации и формул. Нормативная поверхность текущей аксиоматики задаётся документацией и `tests/mtc_formulas.mtc`.

## Структура репозитория

```text
anum_docs/
├── README.md
├── core/
├── parsers/
├── converters/
├── tests/
├── docs/
│   ├── theory/
│   ├── specs/
│   └── research/
├── faq/
├── pics/
└── pdf/
```

## English Summary

MTS is a theory of links. The current working version separates meaning notation, template search, anumber serialization, and ostensive link structures. The root formula is:

```text
∞ : {} = {} ⟼ {}
```

The canonical formula fixture is `tests/mtc_formulas.mtc`.

## Лицензия

Проект распространяется под лицензией [Unlicense](LICENSE).

## Авторы и соавторы

- [Вертушкин Роман Павлович](https://github.com/netkeep80)
- [Дьяченко Константин Константинович](https://github.com/konard)
- [Шакиров Тимур Эдуардович](https://github.com/TimaxLacs)
- [Бурдуков Александр Николаевич](https://github.com/InAiwetrustAGI)
- [Глазунов Иван Сергеевич](https://github.com/ivansglazunov)
