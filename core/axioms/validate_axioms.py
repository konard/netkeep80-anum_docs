...
class MTCAxiomValidator(object):
    ...
    def test_self_closure_consequences(self):
        """Тестирование следствий аксиом конечности и начальности (issue #23, #30)"""
        print("\n=== СЛЕДСТВИЯ АКСИОМ КОНЕЧНОСТИ И НАЧАЛЬНОСТИ ===\n")

        # Начало смысла — следствие А6, не определение
        result1 = self.log_test(
            "Начало смысла — следствие А6 (начальность)",
            True,
        ...
        result2 = self.log_test(
            "Конец смысла — следствие А5 (конечность)",
            True,
            "♂∞ = ♂∞ ⟼ ∞ — частный случай А5 (♂v : ♂v ⟼ v) при v=∞, использует = а не :"
        )

        # А5 вводит понятие конечности через самозамыкание начала
        result3 = self.log_test(
            "А5 вводит понятие конечности через самозамыкание начала",
            True,
            "♂v : ♂v ⟼ v — если нет начала, то остаётся только конец (самозамыкание начала)"
        )

        # А6 вводит понятие начальности через самозамыкание конца
        result4 = self.log_test(
            "А6 вводит понятие начальности через самозамыкание конца",
            True,
            "r♀ : r ⟼ r♀ — если нет конца, то остаётся только начало (самозамыкание конца)"
        )
