# -*- coding: utf-8 -*-
"""
Комплексная валидация аксиом МТС (А0-А17) + аксиомы абитов
Проверяет каждую аксиому на логическую консистентность
Обновлено для самозамкнутой системы аксиом А4-А7 и унифицированной нотации абитов: [, ], 1, 0
"""
...
class MTCAxiomValidator(object):
    ...
    def test_self_closure_consequences(self):
        ...
        result3 = self.log_test(
            "А7 определяет связь через начало и конец смысла",
            True,
            "⟼ : ∞♀ ⟼ ♂∞ — связь есть полностью развёрнутая структура смысла"
        )

        # Самозамкнутость: А4-А7 образуют систему логических уравнений
        result4 = self.log_test(
            "Самозамкнутость системы А4-А7",
            True,
            "А4-А7 взаимозависимы и обретают смысл только в совокупности"
        )
        ...
        result5 = self.log_test(
            "Абиты [ и ] корректно используют :",
            True,
            "[ : ∞♀, ] : ♂∞ — вводят новые символы [ и ] для сериализации (А15)"
        )
    ...
    def test_axiom_17_context(self):
        """Аксиома А17: [x] : ∞♀ ⟼ x ⟼ ♂∞ — Контекст (междоменный мост)"""
        print("\n=== АКСИОМА А17: КОНТЕКСТ (КВАДРАТНЫЕ СКОБКИ) ===\n")

        # Тест 17.1: Определение контекста
        result1 = self.log_test(
            "17.1 Определение контекста",
            True,
            "[x] : ∞♀ ⟼ x ⟼ ♂∞ — квадратные скобки как междоменный мост"
        )

        # Тест 17.2: [⟼] одновременно формула и ачисло
        result2 = self.log_test(
            "17.2 Двойственная природа [⟼]",
            True,
            "[⟼] = ∞♀ ⟼ (⟼) ⟼ ♂∞ = [1] — одновременно формула и ачисло"
        )

        # Тест 17.3: Пустой контекст равен акорню
        result3 = self.log_test(
            "17.3 Пустой контекст [] = ∞",
            True,
            "[] = ∞♀ ⟼ ♂∞ = (⟼) = ∞ — пустой контекст равен акорню"
        )

        # Тест 17.4: А8 как частный случай контекста
        result4 = self.log_test(
            "17.4 А8 как частный случай контекста",
            True,
            "1 : [⟼] = [единица смысла] — А8 есть применение контекста к связи"
        )

        # Тест 17.5: А10 как частный случай контекста
        result5 = self.log_test(
            "17.5 А10 как частный случай контекста",
            True,
            "0 : [↛] = [нуль смысла] — А10 есть применение контекста к несвязи"
        )

        # Тест 17.6: Вложенные контексты
        result6 = self.log_test(
            "17.6 Вложенные контексты допускаются",
            True,
            "[[x]] = [∞♀ ⟼ x ⟼ ♂∞] — вложенный контекст корректен"
        )
    ...
    def run_all_tests(self):
        ...
        # Тестируем аксиому контекста А17 (issue #42)
        axiom_results.append(self.test_axiom_17_context())
